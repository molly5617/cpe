...
(current-level ?c)
(team (id ?a)(choice $?front1 ?advisor_id $?back1))
(test (eq (length$ $?front1)(- ?c 1)))
(not (and
    (team (id ?b)(choice $?front2 ?advisor_id $?back2))
    (test (eq (length$ $?front2)(- ?c 1)))
    (advisor (id ?advisor_id)(choice $? ?b $? ?a $?))
    (not (match (team ?b)(advisor $?)))
))
(not (match (team $?)(advisor ?advisor_id)))
(not (match (team ?a)(advisor $?)))
=>
(assert (match(team ?a)(advisor ?advisor_id)))
)
