...
(defrule match-pay
    (declare (CF 1.0))
    ?f1 <- (job (ID ?id) (pay High))
    =>
    (assert (high-pay ?id)))

(defrule match-working-hours
    (declare (CF 0.7))
    (job (ID ?id) (working-hours Low))
    =>
    (assert (fine-working-hours ?id)))

(defrule match-away-from
    (declare (CF 0.5))
    (job (ID ?id) (away-from Near))
    =>
    (assert (fine-away-from ?id)))

(defrule match-company-profit
    (declare (CF 0.1))
    (job (ID ?id) (company-profit High))
    =>
    (assert (fine-company-profit ?id)))

(defrule match-company-stock
    (declare (CF 0.2))
...
(defrule print
    ?f0 <- (job (ID ?id))
    ?f1 <- (high-pay ?id)
    ?f2 <- (fine-working-hours ?id)
    ?f3 <- (fine-away-from ?id)
    ?f4 <- (fine-company-profit ?id)
